...
You are given a **singly linked list**, an integer `k` representing a position (using **0-based indexing**), and an integer value `data`.  
Your task is to **insert a new node with value `data` at the kth position** in the linked list and return the updated head.

### Note
- Indexing is **0-based**
- Inserting at `k = 0` means inserting at the **head**
...
**Explanation:**  
The value `6` is inserted at index `2` (3rd node in 0-based indexing).
...
- `k` – position where the node should be inserted (0-based)  
